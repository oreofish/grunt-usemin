...
var getBlocks = function (content) {
  ...
  var regjadebuild = /\/\/\s*build:(\w+)(?:\(([^\)]+)\))?\s*([^\s]+)/;
  // end build pattern -- <!-- endbuild -->
  var regend = /<!--\s*endbuild\s*-->/;
  var regjadeend = /\/\/\s*endbuild/;
  ...
  lines.forEach(function (l) {
    ...
    var jadebuild = l.match(regjadebuild);
    var endbuild = regend.test(l);
    var endjadebuild = regjadeend.test(l);
    ...
    if (!endbuild && endjadebuild) {
      endbuild = endjadebuild
    }
